...
describe('absolute URLs', function () {
  ...
  it('should not convert URL', function () {
    ...
    assert.equal('<html><head></head><body><a href="http://example.com/bar.html"></a></body></html>', absolute(path, $el).html())
  })
...
describe('absolute URLs with <base> tag', function () {
  ...
  it('should convert relative URL', function () {
    ...
    const result = absolute(path, $el).html()
    assert(result.includes('<a href="http://example.com/foo/foobar.html"></a>'))
  ...
  it('should not convert relative URL starting with /', function () {
    const $el = cheerio.load(head + '<a href="/foobar.html"></a>')
    const result = absolute(path, $el).html()
    assert(result.includes('<a href="http://example.com/foobar.html"></a>'))
    assert(result.includes(head))
  })
})
